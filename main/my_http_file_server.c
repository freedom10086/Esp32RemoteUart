...
static esp_err_t http_resp_dir_html(httpd_req_t *req, const char *dirpath) {
    ...
    httpd_resp_sendstr_chunk(req, "<!DOCTYPE html><html>"
                                  ...
                                  "<title>SmartBoxFileManage</title></head><body>");
    ...
    httpd_resp_sendstr_chunk(req,
                             "<table class=\"fixed\" border=\"1\" style=\"border-spacing: 0;\">"
...
static esp_err_t set_content_type_from_file(httpd_req_t *req, const char *filename) {
    ...
    } else if (IS_FILE_EXT(filename, ".manifest")) {
        return httpd_resp_set_type(req, "text/cache-manifest");
    }
...
esp_err_t register_file_server(const char *base_path, httpd_handle_t server) {
    ...
    ESP_LOGI(TAG, "File server register successful!");
    return ESP_OK;
}

esp_err_t unregister_file_server(httpd_handle_t server) {
    if (!server_data) {
        ESP_LOGE(TAG, "File server not started");
        return ESP_ERR_INVALID_STATE;
    }
    ...
    ESP_LOGI(TAG, "File server unregister...");
    return ESP_OK;
}
